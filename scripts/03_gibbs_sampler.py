...
parser.add_argument("-b", action="store", dest="beta", type=float, default=50.0,
                    help="Weight on pseudo count (default: 50.0)")
...
parser.add_argument("-i", action="store", dest="iters_per_point", type=int, default=6,
                    help="Number of iteration per data point (default: 6)")
...
def load_peptide_data():
    ...
    raw_peptides = np.loadtxt(training_file, dtype=str).reshape(-1, 2)
...
def to_psi_blast(matrix, out_file_name):
    ...
    print(
        '{:>4} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8}'.format(
            *header), file=output_file)
...
log_odds_matrix, peptide_scores, _ = get_log_odds(peptides, alphabet, bg, blosum62, core_len, c_matrix, f_matrix,
                                                  g_matrix, p_matrix, w_matrix)
...
for t in T:
    ...
    for i in range(0, iters):
        ...
        if len(peptide) != core_len:
            ...
            # get base log_odds
            log_odds_matrix, peptide_scores, p_matrix = get_log_odds(peptides, alphabet, bg, blosum62, core_len,
                                                                     c_matrix, f_matrix, g_matrix, p_matrix, w_matrix)
...
# Testing part
testing = np.loadtxt(test_file, dtype=str).reshape(-1, 2)
